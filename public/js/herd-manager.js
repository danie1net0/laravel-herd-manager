function herdManager() {
  return {
    ...
    focusPortInput(site) {
      ...
      const siteIndex = this.filteredSites.findIndex(
        (filteredSite) => filteredSite.name === site.name
      );
    ...
    validatePortsBeforeApply() {
      ...
      if (hasPortConflicts) {
        this.showErrorMessage(
          "Some ports are in use by the system. Choose different ports."
        );
        return false;
      }
      ...
      if (hasDuplicatePorts) {
        this.showErrorMessage(
          "Multiple sites are using the same port. Choose different ports."
        );
        return false;
      }
    ...
    hasPortConflicts() {
      const sitesWithConflicts = this.sites.filter(
        (site) => site.exposed && site.portInUse
      );
    ...
    async checkForUpdates() {
      try {
        ...
        this.showUpdateBanner = data.update?.available === true;
      } catch (error) {
